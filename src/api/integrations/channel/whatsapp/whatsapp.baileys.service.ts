  ...
  public async whatsappNumber(data: WhatsAppNumberDto) {
    ...
    const verifiedUsers = await Promise.all(
      jids.users.map(async (user) => {
        ...
        // Brazilian numbers - Enhanced logic to handle duplicated 9 digit
        if (user.number.startsWith('55')) {
          // Remove @s.whatsapp.net if it exists to work only with numbers
          const cleanNumber = user.jid.replace('@s.whatsapp.net', '').replace('+', '');

          // Generate all possible variations of the number
          const variations: string[] = [];

          // Case 1: Number with 14 digits (55 + 88 + 99 + 6894405) - with duplicated 9
          if (cleanNumber.length === 14) {
            // Remove the first duplicated 9: 5588996894405 -> 558896894405
            variations.push(cleanNumber.slice(0, 4) + cleanNumber.slice(5));
            // Remove the second duplicated 9: 5588996894405 -> 558896894405
            variations.push(cleanNumber.slice(0, 5) + cleanNumber.slice(6));
            // Keep original
            variations.push(cleanNumber);
          }

          // Case 2: Number with 13 digits (correct: 55 + 88 + 9 + 96894405)
          if (cleanNumber.length === 13) {
            variations.push(cleanNumber); // With 9
            variations.push(cleanNumber.slice(0, 4) + cleanNumber.slice(5)); // Without 9 (12 digits)
          }

          // Case 3: Number with 12 digits (without 9: 55 + 88 + 96894405)
          if (cleanNumber.length === 12) {
            variations.push(cleanNumber); // Without 9
            variations.push(`${cleanNumber.slice(0, 4)}9${cleanNumber.slice(4)}`); // With 9 (13 digits)
          }

          // Remove duplicates and add @s.whatsapp.net
          const uniqueVariations = [...new Set(variations)].map((v) => `${v}@s.whatsapp.net`);

          // Search for any variation that exists
